...
language model, validates the extracted directives, then solves the minimum-cost
...
```text
run.py                 uvicorn entrypoint (binds 0.0.0.0, reads PORT)
app/                   API, interpreter, guardrails, optimizer, validator
data/                  organizer fixture and request/response examples
docs/                  verification, deployment, submission and change guides
scripts/               live/offline public-sample runner
tests/                 pytest suite (mocked LLM, guardrails, optimizer oracle)
```

## Quickstart

Requires **Python 3.12** and network access to your chosen model provider.
Run these commands from the `gridwise` folder of a fresh clone:
...
without replacing already-defined environment variables. A single worker is
intended for this small workload and its in-memory cache.
...
live LLM response, and equivalent optimal battery schedules may differ.
...
| 500 | Controlled model/solver/validation failure; no fabricated schedule |

Response headers include `X-Interpreter-Sources` (`llm:primary`, `llm:backup`
or `cache:llm`), `X-Plan-Status` and `X-Self-Check`. These are diagnostics; the
JSON body follows the official contract. `/health` never makes a paid model
request — check real credentials and quota with the live sample command below.

## Configuration

The shipped example selects Groq's `openai/gpt-oss-120b`, listed in its
[supported models](https://console.groq.com/docs/models). Obtain a valid key and
confirm model access/quota for your account. Provider costs and quotas are your
responsibility; this project makes no free-tier availability guarantee.
...
| `LLM_TIMEOUT_S` | Per-provider-attempt wall-clock limit, default 8 s, maximum 12 |
| `LLM_MAX_TRIES` | Attempts per provider, default 2, maximum 4 |
| `INTERPRET_BUDGET_S` | Shared interpretation deadline, default 20 s, maximum 22 |
...
those parameters. The Anthropic preset uses `/v1/messages`; other presets use an
OpenAI-compatible `/chat/completions`. Protocol handling is tested with mocked
HTTP responses, not with credentials for every vendor; check model availability
with the provider and the live tests.
...
Inside a container, `localhost` means that container, so set `LLM_BASE_URL` to
the reachable model host. A local model must still meet the 30-second judging
timeout; a rule-based parser is not a compliant substitute.
...
2. `interpreter.py` sends quoted notes and battery context to the LLM in one
   request. Its system prompt defines six directive types, end-exclusive time
   windows, solar remaining fractions, percentages, distractors and units.
3. `guardrails.py` checks note-derived fields, supported types, semantics, time
   windows and numeric bounds. It expands windows, sorts and deduplicates
   hours, and converts model-extracted capacity percentages to kWh. Conflicting
   fields trigger a repair attempt.
4. Invalid entries are retried with their original indices and full note
   ...
   timing margin, or use exponential backoff when no header is present. Waiting
   and network attempts share the same bounded time budget; a cooldown beyond
   the deadline never triggers an immediate retry.
...
Only accepted LLM answers are cached. Cache keys cover the complete note list
and all battery context, avoiding stale percentage conversions or a note reused
with different surrounding text. The API never loads test fixtures and uses no
regular-expression fallback. If all models fail, the API returns a controlled
error instead of silently making an important note `no_op`.

## Optimization model

For each hour, variables are grid import `g`, used solar `s`, battery charge
`c`, discharge `d`, and stored energy `e` at the end of the hour. All are
non-negative.
...
The objective has no additive cycling penalty, so small tariff differences are
preserved. A positive scale normalizes the cost vector. In the lossless battery
model, any simultaneous LP charge/discharge is netted into a single action
without changing energy balance, stored energy or grid cost. Curtailment is
allowed and export is forbidden. The solver uses a 3-second internal limit; the
post-parse request pipeline has a 27-second wall-clock budget. These are limits,
not guarantees about latency on an overloaded host.
...
validity and compares cost against all 10 reference optima. Automated tests also
exercise the API with mocked model replies, malformed requests, provider
outages, `Retry-After` rate limits, guardrails, caching and an independent
discrete-state cost oracle. Mocked model tests do not measure language
understanding. The live command checks extracted fields and independently
replays the plan against the **organizer's** directives, then checks cost; it
exits nonzero on failure and reports sample latency. Explanation wording and the
exact schedule are not compared byte-for-byte.
...
the tested image to your registry:
...
raw prompts and model replies are never logged by the application.

## Security notes

- `.env`, `.env.*`, `.env_*`, editor-history backups (`.history/`), cache and
  report directories are Git-ignored at both repository root and here.
- API keys live only in configuration, in memory, and outbound HTTP headers;
  they are never logged or returned by the API.
- Treat any key that ever landed in git history as compromised and rotate it.
...
- Caching is in memory, process-local and lost on restart. Live latency and
  failure rate depend on provider availability, real-account rate limits and
  host load.
